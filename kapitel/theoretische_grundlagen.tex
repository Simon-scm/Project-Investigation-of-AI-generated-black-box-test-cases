...
\subsection{Softwaretesting und Testfälle}
Der Bereich des Softwaretestings beschäftigt sich damit, Systeme und Programme 
...
Allgemeiner formuliert besteht das Ziel von Softwaretesting darin, 
...
und Black-Box-Testing einteilen \cite{nidhra_black_2012}. Für das White-Box-Testing benötigen
Tester und Testfallersteller Zugang zum Quellcode des Programms. Die Tests überprüfen primär die 
...
\subsection{\acs{KI}-gestützte Testfallgenerierung}
Diese Projektarbeit baut wesentlich auf dem Konzept des Black-Box-Testings auf. Vor dem Hintergrund, 
dass Large Language Models (\acs{LLM})\acused{LLM} zunehmend zur Generierung von Quellcode eingesetzt werden, liegt es nahe, 
sie zugleich auch Tests und Testfälle erstellen zu lassen.
So werden Large Language Models inzwischen auch für die Vorbereitung von
...
\ac{LLM} Testfälle ausschließlich aus der natürlichsprachlichen Spezifikation ableiten 
...
und erwartetem Ergebnis, besonders relevant. Beide Angaben werden von dem verwendeten \ac{LLM} 
auf Basis einer natürlichsprachlichen Programmieraufgabe (Spezifikation des Sollverhaltens) erzeugt.
Die empirische Forschung zeigt jedoch, dass die Korrektheit des vom 
\ac{LLM} generierten erwarteten Ergebnisses je nach Aufgabe und Untersuchungsaufbau deutlich variieren kann
...
Ein \ac{KI}-generierter Testfall wird in dieser Arbeit demnach erst als valide eingestuft, wenn die 
Eingabe die jeweiligen Einschränkungen innerhalb der Programmieraufgabe erfüllt und das vom \ac{LLM} 
...
in einer sprachlichen Sequenz miteinander in Beziehung zu setzen \cite{vaswani_attention_2017}. 
Auf dieses Projekt bezogen, können somit Zusammenhänge zwischen den Bedingungen einer natürlichsprachlichen 
Programmieraufgabe, den darin definierten Eingabegrenzen und dem geforderten 
...
das Modell eine Wahrscheinlichkeitsverteilung für das jeweils nächste Token \cite{vaswani_attention_2017}. 
Die Ausgabe eines generativen Sprachmodells stellt folglich keine nachweislich korrekte Aussage dar. 
...
zwar in Verbindung setzen und als Kontext in die Ausgabe mit einfließen lassen kann, jedoch nicht 
garantiert werden kann, dass die Ausgabe alle Vorgaben befolgt und fachlich korrekt ist \cite{huang_survey_2025}.  

Für die Umsetzung des zu dieser Projektarbeit zugehörigen Assistenzsystems wird das von OpenAI bereitgestellte 
Modell \acs{GPT}-4.1 mini verwendet. Die Wahl dieses Modells 
...
Vielmehr wird untersucht, welche Qualität mit der konkreten Kombination aus dem Modell \acs{GPT}-4.1 mini und dem verwendeten System-Prompt 
