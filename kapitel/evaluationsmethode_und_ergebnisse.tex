...
Spektrum an unterschiedlichen Problemarten und möglichen Sonderfällen abzudecken. Es wurden insgesamt sieben Probleme für die Evaluation des Assistenzsystems ausgewählt.
Die genauen Aufgabenstellungen und die daraus generierten Testfälle können dem Anhang~\ref{anhang:programmieraufgaben-testfaelle} entnommen werden.
...
die im weiteren Verlauf dieser Untersuchung als Vergleichsgrundlage für die Validierung der \ac{KI}-generierten erwarteten Ergebnisse dient. 

Die bewusst fehlerhaften Lösungen basieren ebenfalls auf eigenen früheren Lösungsversuchen zu den jeweiligen Problemen. Für die Untersuchung ist das 
vorteilhaft, da es sich nicht um künstlich konstruierte oder absichtlich eingebaute Fehler, sondern um tatsächliche, während des menschlichen Problemlösungsprozesses 
entstandene Implementierungsfehler handelt. Diese falschen Lösungsversuche liefern für viele typische Testfälle das korrekte Ergebnis, scheitern jedoch an spezifischen Grenz- 
oder Sonderfällen. Dadurch eignen sie sich besonders dafür, zu überprüfen, wie gut die \ac{KI}-generierten Testfälle fehlerhafte Implementierungen entlarven.
\clearpage
...
Zu jeder der sieben Programmieraufgaben wurden 15 Testfälle durch das Assistenzsystem generiert. 
...
Das daraus resultierende Ergebnis wurde anschließend mit dem von der \ac{KI} erwarteten Ergebnis verglichen. 
Die Ergebnisse der Ausführung sowie die dazu genutzten Referenzimplementierungen befinden sich in der Anlage \texttt{test-results.md}.
...
Bei der manuellen Constraint-Prüfung konnten insgesamt zehn unzulässige
Testfälle identifiziert werden. Davon entfielen zwei auf Problem 1 und die restlichen 
acht auf Problem 5. Bei Problem 1 wurden mit \(n=2\,863\,311\,530\) und \(n=0\) Werte außerhalb des
...
Damit erfüllen 95 von 105 Testfällen die in den Aufgaben gesetzten Eingabebeschränkungen, 
was einer Constraint-Konformitätsrate von 
...
wurden korrekt berechnet. Für alle anderen Probleme erreichten die von der \ac{KI} generierten Testfälle nach Entfernung
unzulässiger Eingaben eine Sollwertvalidität von \(100\,\%\).

\subsubsection{Interpretation der Validitätsergebnisse}
Aus den Ergebnissen der Validitätsprüfung lassen sich erste interessante Schlüsse ziehen.
Auffällig ist, dass innerhalbn der untersuchten Stichprobe kein Zusammenhang zwischen der 
Constraint-Konformitätsrate und der Sollwertvaliditätsrate zu erkennen ist. Bei Problem 5 lag die 
Constraint-Konformitätsrate mit \(46{,}67\,\%\) deutlich unter dem Durchschnitt, die Sollwertvaliditätsrate 
der generierten Testfälle zu diesem Problem lag jedoch bei \(100\,\%\). 
Umgekehrt lag die Constraint-Konformitätsrate bei Problem 2 bei \(100\,\%\) und die Sollwertvaliditätsrate 
nur bei \(46{,}67\,\%\). Die Untersuchung deutet folglich darauf hin, dass eine geringe Constraint-Konformitätsrate 
nicht zwangsläufig eine geringe Sollwertvaliditätsrate impliziert und umgekehrt.

Insbesondere die acht constraint-widrigen Testfälle für Problem 5 scheinen bemerkenswert. 
Trotz eines klar begrenzten Wertebereichs von 1 bis \(10^9\) enthielten über die Hälfte der generierten Testfälle den Wert 0. 
...
In diesem Fall wurde das Grenzfallmuster jedoch fehlerhaft auf eine Aufgabe angewandt, deren Aufgabenbeschreibung diesen Fall eindeutig 
...
Diese Beobachtung legt die Vermutung nahe, dass die Validität von der Komplexität der semantischen Anforderungen der jeweiligen Aufgabe
anhängt. Damit ist die Schwierigkeit gemeint, die in der Aufgabenbeschreibung formulierten Regeln korrekt 
...
Maßgeblich für die Sollwertvalidität scheint somit nicht die rechnerische Schwierigkeit einer Aufgabe zu sein, sondern die Anzahl, 
Abhängigkeit und sprachliche Eindeutigkeit der anzuwendenden Regeln.

Der einzelne Sollwertfehler bei Problem 3 ist insofern interessant, als dass bei vergleichbaren Testfällen zu derselben Aufgabe keine fehlerhaften Werte für das erwartete 
Ergebnis auftraten. 
...
Die fehlerhaften Implementierungen zeichnen sich dadurch aus, dass sie einen vorab bekannten Defekt enthalten, 
der jedoch nur bei bestimmten Eingaben zu einem abweichenden Ergebnis führt.
Die Ergebnisse dieser Ausführung sowie die fehlerhaften Implementierungen können ebenfalls der Anlage \texttt{test-results.md} entnommen werden.
Ein Testfall erkennt eine falsche Implementierung, wenn das im Testfall definierte erwartete Ergebnis 
...
Sollwert abweichenden Ergebnis. Tabelle~\ref{tab:fehlererkennung-probleme} zeigt, dass sich die 
...
\(85{,}71\,\%\). Nur für Problem~5 wurde durch das \ac{LLM} kein valider Testfall erzeugt, der den eingebauten
...
insbesondere solche Testfälle mit einzubeziehen. Hierbei darf jedoch nicht außer Acht gelassen werden, dass die verwendeten 
Fehlerimplementierungen häufig genau durch ein Eingabemuster entlarvt werden. Aus dieser Sicht wäre ein Ergebnis von 
einem fehlererkennenden Testfall pro Aufgabe als ausreichend zu bewerten, da das \ac{LLM} so nicht weitere Testfälle für den gleichen 
Sonderfall verwendet. Außerdem muss berücksichtigt werden, dass nicht ausgeschlossen werden kann, dass unter den verbleibenden 
Testfällen solche dabei sind, die möglicherweise andere, von dem \ac{LLM} antizipierte Fehlerimplementierungen identifizieren 
würden.

Die gemeinsame Betrachtung der Fehlererkennung und Validität zeigt außerdem einen weiteren interessanten Zusammenhang.
Auffällig ist, dass für Problem 2 ein Testfall generiert wurde, der die falsche Lösung erkennt, obwohl das \ac{LLM} zuvor Schwierigkeiten 
...
Testfall generiert, der die falsche Implementierung erkennen konnte. Hier hatte das \ac{LLM} vorab Probleme, die mitgegebenen Eingabebeschränkungen 
(Constraints) zu befolgen, konnte aber die Sollwerte der Testfälle zu \(100\,\%\) korrekt vorhersagen. Diese Beobachtung legt für die untersuchte 
Stichprobe die Vermutung nahe, dass die Generierung fehlererkennender Testfälle weniger von der semantischen Komplexität der Aufgabenstellung als 
vom Verständnis der vorgegebenen Eingabebeschränkung abhängt.
